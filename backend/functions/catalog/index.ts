import { requireAuth, createAdminClient, corsHeaders, jsonResponse, errorResponse } from "../_shared/auth.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "discovery" || action === "search") {
      const providers = await buildProviders(admin, body.lat ?? null, body.lng ?? null);
      const filtered = applyFilters(providers, body.query, (body.filters ?? {}) as Filters);
      const sorted = sortProviders(filtered, body.sort);
      ...
      return jsonResponse({ providers: sorted });
    }
    ...
    if (action === "clinic") {
      ...
      if (clinicProviders.length === 0) {
        ...
        return jsonResponse({ business: biz, doctors: [], reviews: [], rating: 0, reviewCount: 0 });
      }
    ...
    if (action === "doctor") {
      ...
      const { data: reviews } = await admin
        ...
        .order("createdAt", { ascending: false })
        .limit(50);
      return jsonResponse({ doctor: doctor ?? null, reviews: reviews ?? [] });
    }
    ...
    if (action === "slots") {
      ...
      if (!svc) return jsonResponse({ slots: [] });
